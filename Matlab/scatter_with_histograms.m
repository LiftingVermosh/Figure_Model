function fig = scatter_with_hist(data_matrix, colormap_param)
    % SCATTER_WITH_HIST 绘制散点图并添加直方图以探索分布
    %   输入：
    ...
    %       fig = scatter_with_hist(data, colormap_param);
    ...
    if isempty(colormap_param)
        % 使用默认颜色图（parula），并设置直方图颜色为默认蓝色
        num_colors = 256; % 颜色图分辨率
        cmap = parula(num_colors);
        hist_color = [0, 0.4470, 0.7410]; % MATLAB 默认蓝色
    elseif iscell(colormap_param) && numel(colormap_param) == 2
        ...
        hist_color = color1_rgb; % 直方图使用第一个颜色
    else
        error('colormap_param 必须为空或2元素单元数组');
    end
    
    % 创建图窗
    fig = figure('Color', 'w', 'Name', '散点图与直方图', 'Position', [100, 100, 800, 600]);
    
    % 设置子图位置：主散点图、右直方图、下直方图
    ax_scatter = subplot('Position', [0.1, 0.35, 0.55, 0.55]); % 主散点图
    ax_histy = subplot('Position', [0.7, 0.35, 0.2, 0.55]);   % 右直方图（y分布）
    ax_histx = subplot('Position', [0.1, 0.1, 0.55, 0.2]);    % 下直方图（x分布）
    ...
    % 绘制右直方图（y分布）
    axes(ax_histy);
    histogram(y, nbins, 'Orientation', 'horizontal', ...
        'FaceColor', hist_color, ...
        'EdgeColor', 'none');
    set(ax_histy, 'YAxisLocation', 'right'); % 将 y 轴放在右侧
    grid off;
    % 设置 ylim 与散点图一致
    ylim([y_min_val - 0.1 * y_range_val, y_max_val + 0.1 * y_range_val]);
    yticks([]);
    ...
    % 翻转坐标轴设置
    set(ax_histx, 'XAxisLocation', 'top');  % 将x轴放在上方
    set(ax_histx, 'YDir', 'reverse');       % 反转y轴方向（使y轴向下）
